...
1. **BUG-14**: 修复 `data.json` 中邮箱链接错误（1分钟）
2. **BUG-11**: 补充 404 页面 void_messages 渲染逻辑（15分钟）
3. **BUG-10**: 修复 og:image 路径和文件缺失（10分钟）
4. **删除 `server_new.py`**: 避免混淆和重量（1分钟）

### 🟡 近期修复（中优先级）

5. **BUG-3**: 实现 `cachedData` 失效/刷新机制
6. **BUG-1**: 准备 `cgi.FieldStorage` 的替代方案（Python 升级前）
7. **BUG-2**: 添加文件类型白名单 + 大小限制
8. **CSS 冗余清理**: 合并重复的媒体查询和声明
9. **BUG-9**: 优化 Contact 页面 DOM 操作

### 🟢 长期优化（低优先级）

10. **HTML 模板统一**: 抽取公共 header/footer
11. **admin.html 拆分**: 独立 CSS/JS 文件
12. **CSS 变量化**: 将魔法数字替换为 CSS 变量
13. **TypeScript/ESM 模块化**: script.js 模块化重构
